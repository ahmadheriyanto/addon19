...
class IncomingStagingStockReceipt(models.Model):
    ...
    def action_create_transfer(self):
        ...
        others = recs - inbound - forder

        if inbound:
            try:
                # action_create_transfer_receive already returns a list of results
        ...
        for rec in self.sudo():
            try:
                ...
                if getattr(rec, 'principal_customer_name', False):
                    ...
                    # also supply context default in case create uses default_get somewhere down the flow
